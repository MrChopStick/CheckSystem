...
public class Repair extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		System.out.println(Type+requestData);
		int eqid = 0;
		int repair_id = 0;
		String detail = new String();
		SimpleDateFormat transfer = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss");
		/*
		 * Timestamp report_date = new Timestamp(0); Timestamp m_got_time = new
		 * Timestamp(0); Timestamp m_deal_time = new Timestamp(0); Timestamp
		 * i_got_time = new Timestamp(0); Timestamp i_arrive_time = new
		 * Timestamp(0); Timestamp i_deal_time = new Timestamp(0); Timestamp
		 * finish_time = new Timestamp(0);
		 */
		...
		case "Insert":
			try
			{
				...
				System.out.println("insert"+eqid+detail+time+inspector_id);
				response.getWriter().append(insert_repair_record(eqid, detail, repair_state,inspector_id));
			} catch (Exception e)
			{
				e.printStackTrace();
			}
	...
	String insert_repair_record(int eqid, String detail, int repair_state, int inspector_id)
	{
		String result = new String();

		if (DB_act.Repair_insert(eqid, detail, repair_state, inspector_id))
		{
			result = "OK";
		} else
		{
