  ...
  <script>	
    ...
    function addPlan() { 
       ...
       //window.alert($("mytable").rows[z-1].cell[0].getElementsByTagName("input")[0].value);
		...
		var NewPlan_id=$("Plan_id").value; //获取新增计划编号的值
		var NewPlan_name=$("Plan_name").value;
		var NewPlan_cycle=$("Plan_cycle").value;
		var NewPlan_start_date=$("Plan_start_date").value;
		var NewPlan_start_time=$("Plan_start_time").value;
		var NewPlan_end_time=$("Plan_end_time").value;
		var NewInspector_id=$("Inspector_id").value;
		var NewManager_id=$("Manager_id").value;
		var NewPlan_state=$("Plan_state").value;
		var NewPlan_note=$("Plan_note").value;
		window.alert(z+"h");
